...
exports.fileUpload = async function (file, namePath){
    try {
        //expresion regular para identificar una imagen en base 64
        console.log('THIS IS THE FILE THE PARMAS \n', file);
        let matches = file.match(/^data:([A-Za-z-+\/]+);base64,(.+)$/)
        let response = {};

        console.log('This is matches \n', matches);

        if(matches.length !== 3) return new Error("String invalid");
        ...
        fs.writeFileSync(`${path.dirname(require.main.filename)}/public${fileRoute}`,
        imageBuffer,
        "utf-8"
        );
        return fileRoute;
    } catch (error) {      
        return new Error('Error del servido');
    }
};